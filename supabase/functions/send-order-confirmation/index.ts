...
const ADMIN_EMAIL = 'numan_malik999@yahoo.com';
...
serve(async (req) => {
  ...
  try {
    ...
    const customerEmailHtml = `
      ...
      <p>Hi ${customerFirstName}, we've received your order #${order.id.substring(0, 8)}.</p>
      <h2>Order Summary</h2>
      <table style="width: 100%; border-collapse: collapse;">
        ${itemsHtml}
      </table>
      <p style="text-align: right;"><strong>Subtotal:</strong> ${formatCurrency(order.subtotal)}</p>
      <p style="text-align: right;"><strong>Discount:</strong> -${formatCurrency(order.discount)}</p>
      <p style="text-align: right;"><strong>Shipping:</strong> ${formatCurrency(order.shipping_cost)}</p>
      <h3 style="text-align: right;">Total: ${formatCurrency(order.total_amount)}</h3>
      <p>We'll notify you when your order has shipped.</p>
    `;

    const adminEmailHtml = `
      <h1>New Order Received!</h1>
      <p>A new order has been placed on Breathin.</p>
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: `Breathin <${FROM_EMAIL}>`,
        to: [customerEmail],
        subject: `Your Breathin Order Confirmation #${order.id.substring(0, 8)}`,
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${RESEND_API_KEY}` },
      body: JSON.stringify({
        from: `Breathin Store <${FROM_EMAIL}>`,
        to: [ADMIN_EMAIL],
        subject: `New Order Notification #${order.id.substring(0, 8)}`,
        html: adminEmailHtml,
      }),
    });
